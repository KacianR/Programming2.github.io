...
int addStudent(participant person[]){
	...
	//opens a new student details file and gives an appropriate response
  	if ( (filePtr = fopen("studentDetails.txt", "a+") ) == NULL){
  		printf("New blank file could not be opened!\n");
	}else{
		for(i = 0; i < a; i++){
		    if(person[i].age==0){
		        count++;
			...
			fgetc(stdin);      
		  	...
		  	for(int x =0; x <50; x++){//removes new line from name
		  		if (person[i].Name[x] == '\n'){
		  			person[i].Name[x] = '\0';
				  }
			...
			//inputs data to the file
				fprintf(filePtr, "Student: %s, ", person[i].Name);
				fprintf(filePtr, "IdNumber: %d, Gender: %c, ", person[i].idNum,person[i].gender);
...
void DisplayAll(participant person[]){
	...
	if ((fileReader = fopen("StudentDetail.txt", "r+")) == NULL)
  	{
...
void best_time_Swimming(participant person[]){
	...
	for( i=1; i<a; i++)  {
		if (person[i].age>=6 && person[i].age<=8 && person[i].idNum != temp.idNum ){
...
if(temp2.age== 0){
printf("This person is the winner for Running in Kids of Steel!!\n");
printf("Student: %s\n",temp.Name);
